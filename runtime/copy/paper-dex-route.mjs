...
import {readPoolMetadata} from './pool-metadata-batch.mjs';
import {restoreAuthenticatedPool} from './route-metadata.mjs';
...
export class PaperDexRoute {
 ...
 async pool(hint){
  const trace=this.rpc.context?.getStore()?.quoteTrace,started=performance.now();try{
  const address=addr(hint.address),key=address+':'+hint.topic,cached=this.cache.get(key);if(cached)return cached;if(this.flights.has(key))return await this.flights.get(key);
  const work=this.loadPool(hint,key);this.flights.set(key,work);try{return await work;}finally{this.flights.delete(key);}
  }finally{trace?.contract('POOL_AUTHENTICATION',started,performance.now());}
 }
 async loadPool(hint,key){const address=addr(hint.address);
  const metadata=await readPoolMetadata(this.rpc,address),[[a],[b],[f]]=metadata.base;
  ...
  if(factory===BSC.factory&&(!hint.topic||hint.topic===V2_SWAP)){adapter='PANCAKE_V2';[registered]=await this.rpc.contract(factory,FACTORY,'getPair',[token0,token1]);}
   ...
   [fee]=metadata.fee??await this.rpc.contract(address,PAIR,'fee');adapter=factory===UNI_FACTORY?'UNISWAP_V3':'PANCAKE_V3';[registered]=await this.rpc.contract(factory,V3FACTORY,'getPool',[token0,token1,fee]);
  }else throw Error('PAPER_POOL_FACTORY_UNSUPPORTED');
  check(addr(registered)===address,'PAPER_POOL_NOT_REGISTERED');const p={address,token0,token1,factory,fee:fee===null?null:Number(fee),adapter};this.cache.set(key,p);if(this.rpc.cache)restoreAuthenticatedPool(this.rpc,p);this.rpc.routeMetadataChanged?.();if(this.cache.size>1000)this.cache.delete(this.cache.keys().next().value);return p;
 }
