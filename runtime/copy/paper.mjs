...
export async function processPaperAction(e,action){
 ...
 try{
  ...
  data={...data,event,quote:q,quote_attempts:attempts,timings:times,paper_model:{source:'OWN_CURRENT_ROUTE_QUOTE',output:'MINIMUM_QUOTED_OUTPUT',gas:'ESTIMATED',tax:q.tax_classification,atomic_execution_verified:false,route_limitation:q.live_block??null,near_total_exit_threshold_pct:98,unknown_tax_assumption:q.tax_classification==='UNKNOWN_TAX'?'Quoted output does not establish actual proceeds after unknown transfer taxes':null,historical:!!event.history,late_research:!!late,not_an_onchain_fill:true},comparison:{our_price_bnb:price,target_price_bnb:event.price_quote??null},quote_ms:quoteMs};
  ...
  times.paper_fill_at=Date.now();const fillMono=performance.now(),processMs=fillMono-start;
  ...
  await e.updateAction(row,'REJECTED',{...data,quote_attempts:attempts,route_errors:error.route_errors??[],detection_success:true,execution_success:false},cleanError(error),{reserved:'0'});
