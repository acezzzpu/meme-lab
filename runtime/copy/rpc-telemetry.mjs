...
export class RpcTelemetry {
 ...
 snapshot(providers=[]){
  const now=this.now(),second=Math.floor(now/1000),rows=[...this.groups.values()].map(g=>({...g}));
  ...
  return {at:now,since:this.started,window_precision:'ROLLING_SECONDS',providers:ps,by_method:rows,cache:{hits:this.cacheHits,misses:this.cacheMisses,hit_rate:this.cacheHits+this.cacheMisses?this.cacheHits/(this.cacheHits+this.cacheMisses):null},suppressed:{...this.suppressed},daily:this.daily};
 }
}
