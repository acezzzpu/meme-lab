...
export async function runCopySelfTest(e){
 ...
 for(const role of ['primary','secondary']){
  ...
  add('WS_'+role.toUpperCase(),!!p.ws_url&&row?.status==='CONNECTED'&&Date.now()-row.updated_at<10000,{provider:p.id,health});
 }
 ...
 add('PENDING_TX_STREAM',health.some(h=>(h.pending_seen_count??0)>0),'Requires actual received pending messages; subscription ACK alone is insufficient');
 ...
 const actual=events.filter(r=>r.status==='CONFIRMED'&&!r.data.history),actions=(await e.store.all("SELECT * FROM copy_actions WHERE mode='PAPER' AND state='FILLED'")).map(r=>({...r,data:decode(r.data)})).filter(r=>!r.data.event?.history);
 ...
 add('PANCAKE_QUOTE',actions.some(r=>r.data.quote?.provider?.startsWith('PANCAKE')),'Requires own real market quote');
 add('ZEROX_QUOTE',actions.some(r=>r.data.quote?.provider==='ZEROX_ALLOWANCE_HOLDER'),e.keys?.zeroEx?'No successful measured quote yet':'ZEROX_KEY_NOT_CONFIGURED');
 add('FOT_HANDLING',actions.some(r=>r.data.quote?.tax_classification==='BUY_SELL_TAX'),'Requires real tax-aware quoted fill');
 ...
 const failover=await e.store.setting('copy_failover_proof');add('WS_FAILOVER',!!failover?.passed,failover??'No measured failover test yet');
