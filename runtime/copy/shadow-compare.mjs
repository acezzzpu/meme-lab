...
export async function compareShadow(e,action,event,q,c){
 ...
 try{
  ...
  if(!ok){const failure=sequence?.find(x=>x.status!=='0x1')??call.reason?.rpc_error??call.reason??evidence.simulation.output_validation_error;evidence.failure=classifySimulationFailure(failure,{router:tx.to,token:event.token,route:q.route});}evidence.status=ok?'SIMULATED':'SIMULATION_FAILED';
 }catch(error){evidence.status=evidence.unsigned_tx_built?'SIMULATION_BLOCKED':'BUILD_FAILED';evidence.error=cleanError(error);evidence.failure=classifySimulationFailure(error.rpc_error??error,{stage:evidence.unsigned_tx_built?'SIMULATION':'BUILD',router:evidence.unsigned_transaction?.to??null,token:event.token,route:q.hops?.map(h=>h.adapter)??q.route});}
