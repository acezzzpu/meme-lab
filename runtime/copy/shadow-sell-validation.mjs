...
function requireCalls(result,count,stage){const calls=result?.result?.[0]?.calls;check(calls?.length===count,'SIMULATION_ENVIRONMENT_INCOMPLETE_CALL_SEQUENCE');const failure=calls.find(c=>c.status!=='0x1');if(failure){const error=Error(failure.error?.message??'execution reverted');error.rpc_error=failure.error;error.data=failure.returnData;error.stage=stage;error.call_index=calls.indexOf(failure);error.gas_used=failure.gasUsed;throw error;}return calls;}
...
export async function validateSeededSell(e,event,template,c,{seedBnbRaw='2000000000000000',exactAmountRaw=null}={}){
 ...
 try{
  ...
  const build=buildShadowUnsigned(q,c),gasUsed=BigInt(probeSwap.gasUsed);let low=gasUsed,gasLimit=gasUsed*2n>4000000n?gasUsed*2n:4000000n;check(gasLimit<=8000000n,'SIMULATION_ENVIRONMENT_GAS_BOUND_EXCEEDED');
  Object.assign(evidence,{quote:q,router:build.unsigned_transaction.to,gas_search:[]});
  // Render proved gasUsed +20% can fail inside a nested Flap transfer even when
  // the exact call succeeds with a larger gas allowance. Estimate a verified
  // upper bound in the seeded state; gas consumed is not minimum gas required.
  let checked=await simulate(e,[...prefix,{...strip(build.unsigned_transaction),gas:hex(gasLimit)},balance],parent),calls=requireCalls(checked,6,'FINAL_SELL');
  evidence.gas_search.push({limit_raw:String(gasLimit),status:'PASS'});
  for(let i=0;i<2;i++){
   const trial=(low+gasLimit)/2n,result=await simulate(e,[...prefix,{...strip(build.unsigned_transaction),gas:hex(trial)},balance],parent);
   try{const trialCalls=requireCalls(result,6,'GAS_ESTIMATION');gasLimit=trial;checked=result;calls=trialCalls;evidence.gas_search.push({limit_raw:String(trial),status:'PASS'});}
   catch(error){if(error.call_index!==4)throw error;low=trial;evidence.gas_search.push({limit_raw:String(trial),status:'FAIL',failure:{...classifySimulationFailure({message:error.message,data:error.data,error:error.rpc_error},{stage:'GAS_ESTIMATION',router:evidence.router,token:event.token}),category:'SIMULATION_ENVIRONMENT',root_cause:'GAS_LIMIT_TOO_LOW_AT_IDENTICAL_PINNED_STATE',gas_used_raw:error.gas_used}});}
  }
  const remaining=readBalance(calls[5]),output=credited(calls[4]);
