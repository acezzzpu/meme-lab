...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url,'http://'+req.headers.host);const respond=(data,status=200)=>{if(res.writableEnded)return;res.writeHead(status,{'Content-Type':'application/json','Cache-Control':'no-store'});res.end(JSON.stringify(data));};try{
 ...
 if(url.pathname==='/healthz'){const beat=await store.setting('engine_heartbeat'),desired=await store.setting('engine_desired');return respond({api:'OK',release:'0.4.1-copy-recovery',engine:desired==='RUNNING'?(beat&&Date.now()-beat.at<15000?beat.state:'OFFLINE'):'STOPPED'},desired==='RUNNING'&&(!beat||Date.now()-beat.at>=15000)?503:200);}
