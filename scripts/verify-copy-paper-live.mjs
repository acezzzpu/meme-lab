...
async function finish(){if(stopped)return;stopped=true;clearInterval(timer);await runCopySelfTest(e);await e.stop();await store.set('copy_rpc_telemetry',e.telemetry.snapshot(e.rpc.providers));await save(true);db.close();}
